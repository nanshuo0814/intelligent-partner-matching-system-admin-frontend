    ...
    <div class="box-card">
        ...
        <el-form :inline="true" :model="searchForm" :rules="searchFormRules" class="demo-form-inline"
            ...
            <el-form-item label="队伍名：" prop="name">
                <el-input clearable v-model="searchForm.name" placeholder="请输入队伍名"></el-input>
            </el-form-item>
            <el-form-item label="队伍状态：" prop="status">
                <el-select clearable v-model="searchForm.status" placeholder="请选择队伍状态：">
            ...
            <el-form-item label="队伍描述：" prop="description">
                <el-input clearable v-model="searchForm.description" placeholder="请输入队伍描述："></el-input>
            </el-form-item>
        ...
        <el-table :data="reportList" style="width: 100%">
            ...
            <el-table-column prop="coverImage" label="封面" width="60">
                <template slot-scope="scope">
                    <el-image :src="scope.row.coverImage" style="width: 100%; height: 100%"
                        :preview-src-list="[scope.row.coverImage]"></el-image>
                </template>
            </el-table-column>
            <el-table-column prop="description" label="队伍描述" width="256"></el-table-column>
            <el-table-column label="状态" width="150">
                <template #default="scope">
                    <el-tag v-if="scope.row.status === 0" type="primary">公开</el-tag>
                    <el-tag v-if="scope.row.status === 1" type="success">私有</el-tag>
                    <el-tag v-if="scope.row.status === 2" type="warning">加密</el-tag>
                </template>
            </el-table-column>
            <el-table-column prop="password" label="密码" width="80"></el-table-column>
            <el-table-column prop="currentNum" label="当前人数" width="80"></el-table-column>
            <el-table-column prop="maxNum" label="最大人数" width="80"></el-table-column>
        ...
        <el-dialog :title="editDialogTitle" :visible.sync="editDialogVisible" width="90%">
            <el-form :model="editForm" :rules="editFormRules" ref="editFormRef">
                ...
                <el-form-item label="队伍名：" prop="name">
                    <el-input v-model="editForm.name" placeholder="请输入队伍名："></el-input>
                </el-form-item>
                <el-form-item label="队伍描述：" prop="description">
                    <el-input v-model="editForm.description" placeholder="请输入队伍描述"></el-input>
                </el-form-item>
                <el-form-item label="过期时间：（例如：2003-08-14 22:22:22）" prop="expireTime">
                    <el-input v-model="editForm.expireTime" placeholder="输入过期时间"></el-input>
                </el-form-item>
                <el-form-item label="队伍状态" prop="status">
                ...
                <el-form-item v-if="editForm.status === 2" label="密码：" prop="password">
        ...
        <el-dialog title="确认删除" :visible.sync="deleteDialogVisible" :before-close="handleDeleteDialogClose">
            <div>
                是否确定删除队伍名为（{{
                deleteReport.name
                }}）的队伍？<span style="color: red"> 请谨慎操作！！！</span>
            </div>
            <span slot="footer" class="dialog-footer">
...
export default {
    ...
    methods: {
        ...
        confirmUpdateStatus() {
            ...
            updateTeamInfo({ id, status: trueStatus, content })
                .then((res) => {
                    if (res.code === 20000) {
                        this.searchTags();
